...
Executive::Executive()
{
	...
	for(int i=0; i<3; i++)
	{
		...
		if(i==1)botDifficultySelect.draw_text((W/2)*i+(W/4), H/2-4, "Medium",white, 0, 1, 13);
		if(i==2)botDifficultySelect.draw_text((W/2)*i+(W/4), H/2-4, "Hard",white, 0, 1, 13);
	}
	...
	else
	{
		...
		if(playerBoard.getPlayer() == 1)
		{
			if(botGame)
			{
                ...
                background.draw_text(W/2-30, H/2-4, "Bot is attacking", white, 0, 33);
                disp.display(background);
                bot.botAttack(difficulty,p1Board);
                disp.wait(1000);
                disp.flush();
                loadBoard(p1Board);
                attackComplete = false;
